...
func TestUpdateSingleChunkRetryOnConflict(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("X-Elastic-Product", "Elasticsearch")
		w.Header().Set("Content-Type", "application/json")
		if r.URL.Path == "/ragflow_tenant/_search" {
			_, _ = w.Write([]byte(`{"hits":{"hits":[{"_id":"actual-1"}]}}`))
			return
		}
		if r.URL.Path != "/ragflow_tenant/_update/actual-1" {
			http.NotFound(w, r)
			return
		}
		if r.URL.Query().Get("retry_on_conflict") != "3" || r.URL.Query().Get("refresh") != "wait_for" {
			t.Errorf("update query = %q", r.URL.RawQuery)
		}
		_, _ = w.Write([]byte(`{}`))
	}))
	defer server.Close()
	...
	err = (&Engine{client: client}).updateSingleChunk(t.Context(), "ragflow_tenant", "chunk-1", map[string]interface{}{
